...
module microMotorMount(pulleyDiam = 15, pulleyW = 5, pulleyWallH = 8, pulleyWallW = 2, pulleySlopeW = 5, axleDiam = 1.5) {
    adapterW = 3;
    slideRingW = 1;
    ...
    translate([0,0, totalPulleyW]) motorAdapter(pulleyDiam + pulleyWallH*2, adapterW, axleDiam, glap = glap);
...
module motorAdapter(diam = 10, height = 5, axleDiam = 1.5, glap = 0.1, smoothness = 30) {
    ...
    difference() {
        ...
        // Grip slot
        translate([0, 0, height - gripSlotH])
            intersection() {
...
module pulley(pulleyDiam = 10, pulleyW = 10, pulleyWallH = 10, pulleyWallW = 2, pulleySlopeW = 3, axleDiam = 1.5, wireHoleDiam = 2, glap = 0.1, smoothness = 40) {
    innerR = pulleyDiam/2;
    ...
    difference() {
        ...
        if (wireHoleDiam > 0) {
            translate([axleDiam/2 + wireHoleDiam/2 + 2,0,pulleyWallW + pulleySlopeW + wireHoleDiam/2])  
                rotate([90,0,0])
